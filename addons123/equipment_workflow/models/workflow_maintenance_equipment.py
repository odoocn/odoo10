...
class workflow_maintenance_equipment(models.Model):
    _inherit = "maintenance.scrap"
    ...
    auditor_is_user = fields.Boolean(compute='_get_auditor_is_user', string=u"登录人是处理人")
    ...
    def button_submit(self):
        ...
        if not self.env['ir.model'].search([('model', '=', 'res.workflow')]):
            # self.state = 'done'
            super(workflow_maintenance_equipment, self).approve_expense_sheets()
        # 提交单据，检查是否配置流程
        workflow_list = self.env['res.workflow'].search([('res_model', '=', 'maintenance.scrap')])
        workflow_flg = False
        workflow_id = 0
        for workflow in workflow_list:
            ...
            if self.search([('id', '=', self.id)] + domain):
                ...
                if workflow_dtl:
                    ...
                    # 给处理人发送消息
                    self.message_post(body=_("设备 %s 被 %s 申请报废，审核!") % (self.maintenance_id.name, self.env.user.partner_id.name), subject="RE:设备报废申请",
                                      message_type="comment",
                                      content_subtype='html', subtype_id=1, partner_ids=list(set([user.partner_id.id])))
                    # 更新字段
                    self.update({'workflow_state': True, 'auditor': user, 'rule_sequence': "2", 'state': '2', 'workflow_id': workflow.id})
                    # 创建处理历史和待办
    ...
    def button_reject(self):
        # 将单据退回给创建人、修改状态并发送通知
        self.message_post(body=_("%s 设备的报废申请被 %s 驳回!") % (self.maintenance_id.name, self.env.user.partner_id.name), subject="RE:单据变更提醒",
                          message_type="comment",
                          content_subtype='html', subtype_id=1,
                          partner_ids=list(set([self.create_uid.partner_id.id])))
        self.update({'auditor': self.create_uid, 'rule_sequence': "1", 'state': '4'})
        # 创建处理历史和待办
    ...
    def button_ok(self):
        ...
        if not workflow_dtl:
            ...
            # 审核通过，给申请人发送一个消息
            self.message_post(
                body=_("%s 设备的报废申请单, 已被 %s 审核通过!") % (self.maintenance_id.name, self.env.user.partner_id.name),
                subject="RE:单据变更提醒",
                message_type="comment",
    ...
    def _process_continues(self, workflow_dtl, workflow_id, rule_sequence):
        ...
        user = self._get_user(workflow_dtl)
        # 给处理人发送消息
        self.message_post(body=_("%s 设备的报废申请单等待 %s 审核!") % (self.maintenance_id.name, user.partner_id.name), subject="RE:单据变更提醒",
                          message_type="comment",
                          content_subtype='html', subtype_id=1,
    ...
    def get_formview_id(self):
        ...
        if self.state == "reject":
            return self.env.ref('maintenance.maintenance_scrap_view_form').id
        else:
            return self.env.ref('equipment_workflow.maintenance_scrap_workflow_form').id
