...
class PurchaseOrder(models.Model):
    ...
    user_id = fields.Many2one('res.users', string='采购员', index=True, track_visibility='onchange',
                              default=lambda self: self.env.user)
    # ===zhy
    order_name = fields.Char("订单名称")
    order_code = fields.Char("内部编码")
    ...
    def button_confirm(self):
        for order in self:
            if order.state not in ['draft', 'sent', 'checking']: # update by liyx
                continue
...
class PurchaseOrderLine(models.Model):
    ...
    def change_price(self):
        ...
        if self.product_qty !=0:
            self.price_unit=float(self.price_total_2)/self.product_qty
        for line in self:
            taxes = line.taxes_id.compute_all_2(float(self.price_total_2),line.price_unit, line.order_id.currency_id, line.product_qty, product=line.product_id, partner=line.order_id.partner_id)
            line.update({
                'price_tax': taxes['total_included'] - taxes['total_excluded'],
                'price_total': taxes['total_included'],
                'price_subtotal': taxes['total_excluded'],
    ...
    def show_details(self):
        ...
        view = data_obj.xmlid_to_res_id( 'purchase.view_purchase_operation_details_form_save')
