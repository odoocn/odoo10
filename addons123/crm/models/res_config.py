...
class CRMSettings(models.TransientModel):
    ...
    group_use_analytic = fields.Selection([(0, "不关联"), (1, "关联")], "分析账户", implied_group='crm.group_use_analytic')
    group_use_project = fields.Selection([(0, "不创建"), (1, "创建")], "项目", implied_group='crm.group_use_project')

    @api.onchange('group_use_project')
    def onchange_project(self):
        if self.group_use_project:
            self.module_project = True
            self.group_use_analytic = 0
        else:
            project = self.env['ir.module.module'].search([('name', '=', 'project')])
            if project and project.state == 'uninstalled':
                self.module_project = False

    @api.onchange('group_use_analytic')
    def onchange_analytic(self):
        if self.group_use_analytic:
            self.group_use_project = 0
