...
class Lead(FormatAddress, models.Model):
    ...
    # ===zhy
    start_time = fields.Date('项目开始时间')
    product = fields.Char('产品')
    scope = fields.Char('规模')
    has_budget = fields.Boolean('预算')
    policy_clear = fields.Boolean('甲方决策是否清晰')
    policy = fields.Text('决策流程描述')
    want_bid = fields.Boolean('是否要招标')
    bid = fields.Char('招标方式')
    health = fields.Boolean('项目健康')
    health_situation = fields.Text('健康状况')
    source = fields.Many2one('dvt.crm.source', '商机来源')
    others = fields.Boolean('others', related='source.others')
    source_note = fields.Text('来源说明')
    compare = fields.One2many('dvt.compare.line', 'opportunity', string='竞争对手')
    account_analytic_id = fields.Many2one('account.analytic.account', string=u'分析账号')
    activities = fields.One2many('crm.activity', 'opportunity', string=u'活动')
    activity_count = fields.Integer(compute="compute_activity")

    @api.depends("activities")
    def compute_activity(self):
        for r in self:
            r.activity_count = len(r.activities)
    # ---zhy
    ...
    def create(self, vals):
        ...
        # ===zhy
        if self.env['res.users'].has_group('crm.group_use_analytic'):
            account = self.env['account.analytic.account'].create({'name': vals['name']})
            vals['account_analytic_id'] = account.id
        if self.env['res.users'].has_group('crm.group_use_project'):
            account = self.env['project.project'].create({'name': vals['name']}).analytic_account_id
            vals['account_analytic_id'] = account.id
        # ---zhy
    ...
    def write(self, vals):
        ...
        if vals.get('name') and self.env['res.users'].has_group('crm.group_use_analytic'):
            for crm in self:
                self.env['account.analytic.account'].browse(crm.account_analytic_id.id).write({
                    'name': vals['name']})
    ...
    def retrieve_sales_dashboard(self):
        ...
        for opp in opportunities:
            ...
            # Next activities
            # ===zhy
            for activity in opp.activities:
                if activity.state == 'unfinished':
                    date_end = fields.Datetime.from_string(activity.plane_end_time)
                    date_start = fields.Datetime.from_string(activity.plane_start_time)
                    if datetime.today().replace(hour=0, minute=0, second=0) <= date_start \
                            <= datetime.today().replace(hour=23, minute=59, second=59):
                        result['activity']['today'] += 1
                    if datetime.today().replace(hour=0, minute=0, second=0) <= date_start \
                            <= (datetime.today() + timedelta(days=7)).replace(hour=23, minute=59, second=59):
                        result['activity']['next_7_days'] += 1
                    if date_end <= datetime.utcnow():
                        result['activity']['next_7_days'] += 1
                else:
                    date_start = fields.Datetime.from_string(activity.start_time)
                    if date.today().replace(day=1) <= date_start <= date.today():
                        result['done']['this_month'] += 1
                    elif date.today() + relativedelta(months=-1, day=1) <= date_start < date.today().replace(day=1):
