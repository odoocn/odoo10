...
class HrExpenseType(models.Model):
    ...
    project_account = fields.Boolean(string=u'项目核算', default=False)
    travel = fields.Boolean(string=u'差旅费', default=False)
    ...
    def name_get(self):
        ...
        for r in self:
            special = []
            if r.project_account:
                special.append(u"项目核算")
            if r.travel:
                special.append(u'差旅费')
            name = r.name
            if special:
                for s in special:
                    name += "(" + s + ")"
...
class HrExpense(models.Model):
    ...
    date = fields.Date(related="sheet_id.expense_date", string="Date")
    # ---zhy
    ...
    # ===zhy
    type_id = fields.Many2one('hr.expense.type', string=u"费用类型", readonly=True, required=True,
                              states={'draft': [('readonly', False)], 'refused': [('readonly', False)]})
    start_time = fields.Datetime(string=u'开始时间')
    end_time = fields.Datetime(string=u'结束时间')
    travel = fields.Boolean(string=u'差旅费', related='type_id.travel')
    project_account = fields.Boolean(string=u'项目核算', related='type_id.project_account')
    ...
    #         if not expense.sheet_id:
    #             expense.state = "draft"
    #         elif expense.sheet_id.state in ("cancel", "reject"):
    #             expense.state = "refused"
    #         elif not expense.sheet_id.account_move_id:
    #             expense.state = "reported"
    #         else:
    ...
    def view_sheet(self):
        ...
        form_view_id = imd.xmlid_to_res_id('hr_expense.view_hr_expense_sheet_form')
        return {
...
class HrExpenseProductLine(models.Model):
    _name = "hr.expense.product.line"

    product_id = fields.Many2one('product.product', string=u'产品')
    amount = fields.Monetary(string=u'金额')
    currency_id = fields.Many2one('res.currency', string='Currency', readonly=True,
                                  default=lambda self: self.env.user.company_id.currency_id)
    sheet_id = fields.Many2one('hr.expense.sheet')
    page_id = fields.Many2one('hr.expense.print.page')


class HrExpensePrintPage(models.Model):
    _name = "hr.expense.print.page"

    sheet_id = fields.Many2one('hr.expense.sheet')
    line_ids = fields.One2many('hr.expense.product.line', 'page_id')
    total_amount = fields.Monetary(string=u'总计', compute='compute_total')
    currency_id = fields.Many2one('res.currency', string='Currency', readonly=True,
                                  default=lambda self: self.env.user.company_id.currency_id)
...
class HrExpenseSheet(models.Model):
    ...
    department_id = fields.Many2one('hr.department', string='Department',
                                    states={'post': [('readonly', True)], 'done': [('readonly', True)]})
    # ===zhy
    product_line = fields.One2many('hr.expense.product.line', 'sheet_id', string='按产品合计', compute="compute_line")
    page_ids = fields.One2many('hr.expense.print.page', 'sheet_id', string='按产品合计')
    # ---zhy
    # --------------------------mlp----------------------------
    number = fields.Char(string=u'编号')
    expense_date = fields.Date(string='日期', default=fields.Date.context_today)
    ...
    def compute_line(self):
        for r in self:
            if r.id:
                line_ids = {}
                self.env['hr.expense.product.line'].search([('sheet_id', '=', r.id)]).unlink()
                for line in r.expense_line_ids:
                    if line.product_id.id in line_ids.keys():
                        line_ids[line.product_id.id] += line.total_amount
                    else:
                        line_ids[line.product_id.id] = line.total_amount
                for key in line_ids.keys():
                    self.env['hr.expense.product.line'].create({'sheet_id': r.id,
                                                                'product_id': key,
                                                                'amount': line_ids[key]})
                r.product_line = self.env['hr.expense.product.line'].search([('sheet_id', '=', r.id)])
    ...
    def print_expense(self):
        for page in self.page_ids:
            page.unlink()
        line_id = 0
        page = self.env['hr.expense.print.page'].create({'sheet_id': self.id})
        for line in self.product_line:
            if line_id >= 5:
                line_id = 0
                page = self.env['hr.expense.print.page'].create({'sheet_id': self.id})
            line.write({'page_id': page.id})
            line_id += 1
        expense = self.mapped('page_ids')
        if not expense:
            raise UserError(_('Nothing to print.'))
        return self.env["report"].with_context(active_ids=expense.ids, active_model='hr.expense.print.page'). \
            get_action([], 'hr_expense.expense_page')
    ...
    @api.multi
    def unlink(self):
        for obj in self.expense_line_ids:
            obj.unlink()
        for expense in self:
            if expense.state not in ('draft', 'cancel'):
                raise UserError(_("非草稿或未取消的费用报告不能删除"))
    ...
    def print_expense_sheet(self):
        ...
        worksheet = workbook.add_sheet(u'支付凭证')
        worksheet.col(0).width = 0x0d00 + 7500
        worksheet.col(1).width = 0x0d00 + 12000
        worksheet.col(2).width = 0x0d00 + 7500
        worksheet.write_merge(0, 1, 0, 2, u'支出凭证', self.set_style('Arial', 400, True))
        worksheet.write(2, 0, u'科目:          ', self.set_style('Arial', 250, True))
        worksheet.write(2, 1, u'       年     月     日', self.set_style("Arial", 250, True))
        worksheet.write(2, 2, u'对方科目:          ', self.set_style('Arial', 250, True))
        worksheet.write(3, 0, u'账户', self.set_style('Arial', 300, True))
        worksheet.write(3, 1, u'支出说明', self.set_style('Arial', 300, True))
        worksheet.write(3, 2, u'金额', self.set_style('Arial', 300, True))
        row = 4
        base_style = xlwt.easyxf('align: wrap yes')
        expense = self.env['hr.expense.sheet'].search([('id', '=', self.id)])
        for line in expense.product_line:
            name = line.product_id.product_tmpl_id.name
            amount = line.amount
            worksheet.write(row, 0, name, base_style)
            worksheet.write(row, 2, amount, base_style)
            row += 1
        ...
        return request.make_response(data, headers=[
                ('Content-Disposition', self.content_disposition(self.filename(u'支付凭证'))),
