...
class SaleOrder(models.Model):
    ...
    order_code = fields.Char(string=u'内部编码')
    data_from = fields.Date(string=u'单据日期', required=True, default=fields.Date.context_today)
    # ---zhy
...
class SaleOrderLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            if line.product_uom_qty != 0:
                one_price = '%.2f' % (float(line.price_total_2) / line.product_uom_qty)
    ...
    def _compute_qty_delivered_updateable(self):
        for line in self:
            line.qty_delivered_updateable = (line.order_id.state == 'sale') and (
    ...
    layout_category_sequence = fields.Integer(related='layout_category_id.sequence', string='Layout Sequence',
                                              store=True)
    ...
    def change_price_total(self):
        self.price_total_2 = self.price_total

    @api.multi
    def change_price(self):
        self.price_total = self.price_total_2
        if self.product_uom_qty != 0:
            self.price_unit = float(self.price_total_2) / self.product_uom_qty
        for line in self:
            price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
            taxes = line.tax_id.compute_all_2(float(self.price_total_2), price, line.order_id.currency_id,
    ...
    def show_details(self):
        ...
        view = data_obj.xmlid_to_res_id('sale.view_sale_operation_details_form_save')
        pack = self.browse(self.id)
        return {
    ...
    def _onchange_discount(self):
        ...
        new_list_price = self.env['account.tax']._fix_tax_included_price(new_list_price, self.product_id.taxes_id,
                                                                         self.tax_id)
        ...
        if price != 0 and new_list_price != 0:
            if self.product_id.company_id and self.order_id.pricelist_id.currency_id != self.product_id.company_id.currency_id:
                ...
                new_list_price = self.env['res.currency'].browse(currency_id).with_context(ctx).compute(new_list_price,
