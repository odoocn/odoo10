...
class DAReportGeneralLedger(models.AbstractModel):
    _inherit = 'report.account.report_generalledger'

    def _get_account_res(self, accounts, init_move_lines, move_lines, display_account, source_group_id, source_group_name):
        account_res = []
        for account in accounts:
            ...
            if account.balance_direct == 'credit':
                ...
                direct = 1
            # 期初
            if init_move_lines.get(source_group_id) and init_move_lines.get(source_group_id).get(account.id):
                res['init_balance'] = init_move_lines[source_group_id][account.id][0]['balance'] * direct
                res['init_debit'] = init_move_lines[source_group_id][account.id][0]['debit']
                res['init_credit'] = init_move_lines[source_group_id][account.id][0]['credit']
            else:
                res['init_balance'] = 0.0
                res['init_debit'] = 0.0
                res['init_credit'] = 0.0
            # 期末
            res['move_lines'] = move_lines[source_group_id][account.id]
        ...
        res = False
        if display_account in ('all', 'just'):
            res = {"account": account_res, "name": source_group_name}
...
GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, p.name'''
        ...
        for row in cr.dictfetchall():
            balance = init_move_lines[row['groupid']][row['account_id']][0]['balance'] if init_move_lines.get(row['groupid']) and init_move_lines.get(row['groupid']).get(row['account_id']) else 0
        ...
        group_res = []
        if group_choice == 'nothing':
            res = self._get_account_res(accounts, init_move_lines, move_lines, display_account, 0, False)
            if res:
                group_res.append(res)
        else:
            for source_group in groupby:
                res = self._get_account_res(accounts, init_move_lines, move_lines, display_account, source_group.id,
                                            source_group.name)
                if res:
                    group_res.append(res)
            res = self._get_account_res(accounts, init_move_lines, move_lines, display_account, 0, u"辅助核算项缺失")
            if res:
                group_res.append(res)
        return group_res
