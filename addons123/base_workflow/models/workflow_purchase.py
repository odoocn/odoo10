...
class workflow_purchase(models.Model):
    _inherit = "purchase.order"

    @api.multi
    def _workflow_history_count(self):
        for op in self:
            workflow_history = self.env['res.workflow.history'].search([('model', '=', 'purchase.order'), ('res_id', '=', op.id)])
            op.workflow_history_count = len(workflow_history)

    @api.multi
    def _workflow_history_count_two(self):
        for op in self:
            workflow_history = self.env['res.workflow.history'].search([('model', '=', 'purchase.order'), ('res_id', '=', op.id)])
            op.workflow_history_count_two = len(workflow_history)
    ...
    workflow_history_count_two = fields.Integer(compute='_workflow_history_count_two', string=u'操作历史')
    ...
    def button_submit(self):
        ...
        if not self.env['ir.model'].search([('model', '=', 'res.workflow')]):
            # self.state = 'done'
            super(workflow_purchase, self).button_confirm()
        # 提交单据，检查是否配置流程
        workflow_list = self.env['res.workflow'].search([('res_model', '=', 'purchase.order')])
        workflow_flg = False
        workflow_id = 0
        for workflow in workflow_list:
            ...
            if self.search([('id', '=', self.id)] + domain):
                ...
                if workflow_dtl:
                    ...
                    # 给处理人发送消息
                    self.message_post(body=_("%s 采购订单 %s 等待 %s 审核!") % (self.user_id.employee_id.name, self.name, user.partner_id.name), subject="RE:单据变更提醒",
                    ...
                    # self.state = 'done'
                    super(workflow_purchase, self).button_confirm()  # 确认订单
                break
        if not workflow_flg:
            # self.state = 'done'
            super(workflow_purchase, self).button_confirm()  # 确认订单
    ...
    def button_cancel(self):
        ...
        super(workflow_purchase, self).button_cancel()
        return True
    ...
    def button_reject(self):
        # 将单据退回给创建人、修改状态并发送通知
        self.message_post(body=_("%s 采购订单 %s 被 %s 驳回!") % (self.user_id.employee_id.name, self.name, self.env.user.partner_id.name), subject="RE:单据变更提醒",
                          message_type="comment",
    ...
    def button_ok(self):
        ...
        if not workflow_dtl:
            ...
            # 审核通过，给申请人发送一个消息
            self.message_post(
                body=_("%s 采购订单 %s 已审核通过!") % (self.user_id.employee_id.name, self.name),
                subject="RE:单据变更提醒",
                message_type="comment",
    ...
    def get_formview_id(self):
        ...
        if self.state == "reject":
            ...
            return self.env.ref('base_workflow.purchase_order_workflow_form').id
    ...
    def delete_message_by_state(self, user):
        ...
        messages = self.env['mail.message'].search(
            [('res_id', '=', self.id), ('model', '=', 'purchase.order')]).filtered(
