...
class workflow_requisition(models.Model):
    _inherit = "requisition.pay"

    @api.multi
    def _workflow_history_count(self):
        for op in self:
            workflow_history = self.env['res.workflow.history'].search([('model', '=', 'requisition.pay'), ('res_id', '=', op.id)])
    ...
    def button_submit(self):
        ...
        if not self.env['ir.model'].search([('model', '=', 'res.workflow')]):
            self.state = 'done'
        # 提交单据，检查是否配置流程
        workflow_list = self.env['res.workflow'].search([('res_model', '=', 'requisition.pay')])
        workflow_flg = False
        workflow_id = 0
        for workflow in workflow_list:
            ...
            if self.search([('id', '=', self.id)] + domain):
                ...
                if workflow_dtl:
                    ...
                    # 给处理人发送消息
                    self.message_post(body=_("%s 付款申请 %s 等待 %s 审核!") % (self.create_uid.employee_id.name, self.name, user.partner_id.name), subject="RE:单据变更提醒",
    ...
    def button_reject(self):
        # 将单据退回给创建人、修改状态并发送通知
        self.message_post(body=_("%s 付款申请 %s 被 %s 驳回!") % (self.create_uid.employee_id.name, self.name, self.env.user.partner_id.name), subject="RE:单据变更提醒",
                          message_type="comment",
    ...
    def button_ok(self):
        ...
        if not workflow_dtl:
            ...
            # 审核通过，给申请人发送一个消息
            self.message_post(
                body=_("%s 付款申请 %s 已审核通过!") % (self.create_uid.employee_id.name, self.name),
                subject="RE:单据变更提醒",
                message_type="comment",
    ...
    def _process_continues(self, workflow_dtl, workflow_id, rule_sequence):
        ...
        # 给处理人发送消息
        self.message_post(
            body=_("%s 付款申请 %s 等待 %s 审核!") % (self.create_uid.employee_id.name, self.name, user.partner_id.name), subject="RE:单据变更提醒",
                          message_type="comment",
                          content_subtype='html', subtype_id=1,
    ...
    def get_formview_id(self):
        ...
        if self.state == "reject":
            return self.env.ref('driserp_requisition.requisition_pay_watch_view').id
        else:
            return self.env.ref('base_workflow.requisition_pay_workflow_a_form').id
    ...
    def delete_message_by_state(self, user):
        ...
        messages = self.env['mail.message'].search(
            [('res_id', '=', self.id), ('model', '=', 'requisition.pay')]).filtered(
