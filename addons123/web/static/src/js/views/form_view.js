...
var FormView = View.extend(common.FieldManagerMixin, {
    ...
    do_onchange: function(widget) {
        ...
        if (!_.isEmpty(result.warning)) {
            // 修改 liyx 2016/12/21
            if (!_.isEmpty(result.warning.callback)){
                eval(result.warning.callback)
            }else{
    ...
    // 添加 liyx 2016/12/21
    callback_function: function(result, select_options) {
        var options = ""
        var ttype = result;
        if($("input.model_fields_ttype") != undefined){
            $("input.model_fields_ttype").val(ttype);

            $("input.db_model_condition").val("");
            $("input.model_fields_input").val("");
            $("input.input_condition").val("");

            if(ttype == "integer" || ttype == "float" || ttype == "date" || ttype == "datetime" || ttype == "monetary"){
                options = "<option value=''></option><option value='1'>等于</option><option value='2'>不等于</option><option value='3'>大于</option><option value='4'>小于</option><option value='5'>大于等于</option><option value='6'>小于等于</option><option value='7'>设置</option><option value='8'>未设置</option>";
            }else if(ttype == "boolean"){
                options = "<option value=''></option><option value='11'>为真</option><option value='12'>为假</option>";
            }else if(ttype == "selection"){
                options = "<option value=''></option><option value='1'>等于</option><option value='2'>不等于</option><option value='7'>设置</option><option value='8'>未设置</option>";
            }else{
                options = "<option value=''></option><option value='9'>包含</option><option value='10'>不包含</option><option value='1'>等于</option><option value='2'>不等于</option><option value='7'>设置</option><option value='8'>未设置</option>";
            }
        }
        if($('select.model_condition').length > 0){
            $('select.model_condition').empty();
            $('select.model_condition').append(options);
        }
        if (select_options != undefined){
            if($('input.input_condition').length > 0){
                $('input.input_condition').replaceWith('<span class="oe_form_field oe_form_field_selection input_condition"><select class="input_condition" onchange="select_change(this)"><option value=""></option>'+select_options+'</select></span>');
            }
        }else{
            $('span.input_condition').replaceWith('<input class="o_form_input input_condition o_form_field" type="text" onchange="input_change(this)" />');
            $('input.input_condition').replaceWith('<input class="o_form_input input_condition o_form_field" type="text" onchange="input_change(this)" />');
        }
//        alert($('span.model_fields').find("input").val());
    },
