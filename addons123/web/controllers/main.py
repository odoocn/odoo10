...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            if request.env['identify.date'].sudo().search([]):
                end_dates = request.env['identify.date'].sudo().search([])[0].end_date
                end_date = datetime.datetime.strptime(end_dates,'%Y-%m-%d').date()
            else:
                end_date = datetime.date.today()
            today = datetime.date.today()
            if uid is not False:
                if today > end_date:
                    values['error'] = _("系统已到期，请联系管理员")
    ...
    @http.route('/web/identify/type', type='http', auth="none")
    def identify_type(self,**kw):
        dict={}
        if request.env['identify.date'].sudo().search([]):
            obj=request.env['identify.date'].sudo().search([])[0]
            today=datetime.date.today()
            end_date=datetime.datetime.strptime(obj.end_date,'%Y-%m-%d').date()
            dict['type']=obj.type
            if today>end_date:
                dict['active']=0
            else:
