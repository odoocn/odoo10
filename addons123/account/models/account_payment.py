...
class off_account_line(models.Model):
    _name = "account.off.line"

    name = fields.Many2one('account.account', string=u"差异过账科目", domain=[('deprecated', '=', False)], copy=False,
                           required=True)
    amount = fields.Monetary(string=u'金额', required=True)
    currency_id = fields.Many2one('res.currency', string='Currency', required=True,
                                  default=lambda self: self.env.user.company_id.currency_id)
    payment_id = fields.Many2one('account.payment', string=u'付款单')
...
class account_payment(models.Model):
    ...
    writeoff_account_id = fields.One2many("account.off.line", "payment_id", string="Difference Account")
    ...
    def _create_payment_entry(self, amount):
        ...
        if self.payment_difference_handling == 'reconcile' and self.payment_difference:
            total_amount = 0
            for line in self.writeoff_account_id:
                total_amount += line.amount
            if abs(total_amount - self.payment_difference) > 0.001:
                raise ValidationError(_("差异合计与实际差异不一致"))
            for line in self.writeoff_account_id:
                writeoff_line = self._get_shared_move_line_vals(0, 0, 0, move.id, False)
                debit_wo, credit_wo, amount_currency_wo, currency_id = aml_obj.with_context(date=self.payment_date).compute_amount_fields(line.amount, self.currency_id, self.company_id.currency_id, invoice_currency)
                writeoff_line['name'] = _('Counterpart')
                writeoff_line['account_id'] = line.name.id
