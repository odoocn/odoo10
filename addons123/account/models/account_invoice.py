...
class AccountInvoice(models.Model):
    ...
    def get_parent(self):
        for obj in self:
            obj.parent_id = obj.partner_id.parent_id
    ...
    partner_id = fields.Many2one('res.partner', string='Partner', change_default=True,
                                 ...
                                 track_visibility='always')
    # add mlp ======================
    parent_id = fields.Many2one('res.partner', string=u'上级', compute='get_parent')
    contact_id = fields.Many2one('res.partner', string='Partner', change_default=True,
                                 readonly=True, states={'draft': [('readonly', False)]},
                                 track_visibility='always')
    # ================================================
    ...
    def invoice_validate(self):
        for invoice in self:
            ...
            if float_is_zero(invoice.amount_total_company_signed, precision_digits=precision):
                raise UserError(_("票据总计不能为0，请检查票据内容"))
        return self.write({'state': 'open'})
...
class AccountInvoiceLine(models.Model):
    ...
    price_subtotal_2 = fields.Char(string=u'小计')  # add mlp =======================

    # add mlp ===============================================
    @api.onchange('price_subtotal')
    def price_subtotal_change(self):
        self.price_subtotal_2 = self.price_subtotal

    @api.multi
    def change_price(self):
        self.price_subtotal = self.price_subtotal_2

    @api.multi
    def show_details(self):
        data_obj = self.env['ir.model.data']
        view = data_obj.xmlid_to_res_id('account.view_account_operation_details_form_save')
