...
<p>The following email sent to %s cannot be accepted because this is a private email address.
   Only allowed people can contact us at this address.</p>
    ...
    def message_post(self, body='', subject=None, message_type='notification',
        ...
        # 以下为极光推送, 参量registration_id为新增
        for partner_id in partner_ids:
            try:
                app_info = self.env['jpush.conf'].search([])
                _jpush = jpush.JPush(app_info.app_key, app_info.master_secret)
                _jpush.set_logging("ERROR")
                push = _jpush.create_push()
                # 根据partner_id 取得员工信息中的设备ID

                user = self.env['res.users'].search([('partner_id', '=', partner_id)])
                push.audience = jpush.audience(
                    jpush.registration_id(user.registration_id)
                )
                push.notification = jpush.notification(alert=body, ios={"sound": "sound.caf"})
                push.message = jpush.message(body, title=u'提醒', content_type=None,
                                             extras={"model": values['model'], "id": values['res_id']})
                push.options = {
                    "time_to_live": 60,
