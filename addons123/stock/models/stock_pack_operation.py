...
class PackOperation(models.Model):
    ...
    # ===zhy
    barcode = fields.Char(string=u'条码', related='product_id.barcode', readonly=True)
    # ============mlp----------------------------
    order_date = fields.Datetime(u'单据日期', compute="change_order_date")

    @api.depends('picking_id.order_date')
    def change_order_date(self):
        for pick in self:
            pick.order_date = pick.picking_id.order_date
