...
class PickingType(models.Model):
    ...
    def _compute_last_done_picking(self):
        # TDE TODO: true multi
        tristates = []
        for picking in self.env['stock.picking'].search([('picking_type_id', '=', self.id), ('state', '=', 'done')],
...
class Picking(models.Model):
    ...
    # ===========mlp=======================
    order_date = fields.Datetime(u'单据日期')
    # ----------------------------------------------------
    _sql_constraints = [
    ...
    def onchange_picking_type(self):
        ...
        if self.partner_id:
            ...
            'no-message', 'block') and partner.parent_id.picking_warn == 'block':
                partner = self.partner_id.parent_id
            else:
    ...
    def recompute_remaining_qty(self, done_qtys=False):
        ...
        def _create_link_for_index(operation_id, index, product_id, qty_to_assign, quant_id=False):
            ...
            self.env['stock.move.operation.link'].create(
                {'move_id': move_dict['move'].id, 'operation_id': operation_id, 'qty': qty_on_link,
        ...
        moves = sorted([x for x in self.move_lines if x.state not in ('done', 'cancel')],
