...
class Warehouse(models.Model):
    ...
    # ===zhy
    wh_return_stock_loc_id = fields.Many2one('stock.location', '退货区')
    re_type_id = fields.Many2one('stock.picking.type', '退货单')
    rv_type_id = fields.Many2one('stock.picking.type', '退供应商')
    # ===zhy
    ...
    def create(self, vals):
        ...
        return_loc = imd.xmlid_to_res_id('stock.stock_location_locations_virtual_return')
        vals['wh_return_stock_loc_id'] = return_loc
        # ---zhy
        sub_locations = {
    ...
    def create_sequences_and_picking_types(self):
        ...
        create_data = {
            'in_type_id': {
                ...
                'use_existing_lots': False,
                'default_location_src_id': False,
                'default_location_dest_id': self.wh_return_stock_loc_id.id,
                'color': color
            }, 'rv_type_id': {
                'name': _('退供应商'),
                'code': 'outgoing',
        ...
        PickingType.browse(warehouse_data['out_type_id']).write({'return_picking_type_id': warehouse_data['re_type_id']})
        PickingType.browse(warehouse_data['in_type_id']).write({'return_picking_type_id': warehouse_data['rv_type_id']})
        PickingType.browse(warehouse_data['rv_type_id']).write({'return_picking_type_id': warehouse_data['re_type_id']})
    ...
    def _get_sequence_values(self):
        return {
            're_type_id': {'name': self.name + _(' Sequence return'), 'prefix': self.code + '/RE/', 'padding': 5},
            'rv_type_id': {'name': self.name + _(' Sequence return vendor'), 'prefix': self.code + '/RV', 'padding': 5},
