...
var FieldTextHtmlSimple = widget.extend({
    ...
    commit_value: function () {
        ...
        // ===zhy 离开codeview模式过程会导致实际内容根据codeview存储用的textarea中内容变化
        // ===zhy 是否处于codeview模式加设是否含有codeview样式判断
        if (this.$content.prev('textarea').length === 1 && this.$content.parent().parent('.note-editor').hasClass('codeview')) {
